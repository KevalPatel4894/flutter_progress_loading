...
/// toast position
enum ProgressLoadingToastPosition {
  top,
  center,
  bottom,
}
...
/// loading animation
enum ProgressLoadingAnimationStyle {
  opacity,
  offset,
  scale,
  custom,
...
enum ProgressLoadingIndicatorType {
  fadingCircle,
  circle,
  threeBounce,
  chasingDots,
...
/// loading status
enum ProgressLoadingStatus {
  show,
  dismiss,
}

typedef ProgressLoadingStatusCallback = void Function(
    ProgressLoadingStatus status);

class ProgressLoading {
  /// loading style, default [ProgressLoadingStyle.dark].
  late ProgressLoadingStyle loadingStyle;

  /// loading indicator type, default [ProgressLoadingIndicatorType.fadingCircle].
  late ProgressLoadingIndicatorType indicatorType;

  /// loading mask type, default [ProgressLoadingMaskType.none].
  late ProgressLoadingMaskType maskType;

  /// toast position, default [ProgressLoadingToastPosition.center].
  late ProgressLoadingToastPosition toastPosition;

  /// loading animationStyle, default [ProgressLoadingAnimationStyle.opacity].
  late ProgressLoadingAnimationStyle animationStyle;
  ...
  /// boxShadow of loading, only used for [ProgressLoadingStyle.custom].
  ...
  ProgressLoadingOverlayEntry? overlayEntry;
  GlobalKey<ProgressLoadingContainerState>? _key;
  GlobalKey<ProgressLoadingProgressState>? _progressKey;
  Timer? _timer;

  Widget? get w => _w;
  GlobalKey<ProgressLoadingContainerState>? get key => _key;
  GlobalKey<ProgressLoadingProgressState>? get progressKey => _progressKey;

  final List<ProgressLoadingStatusCallback> _statusCallbacks =
      <ProgressLoadingStatusCallback>[];

  factory ProgressLoading() => _instance;
  static final ProgressLoading _instance = ProgressLoading._internal();

  ProgressLoading._internal() {
    /// set deafult value
    loadingStyle = ProgressLoadingStyle.dark;
    indicatorType = ProgressLoadingIndicatorType.fadingCircle;
    maskType = ProgressLoadingMaskType.none;
    toastPosition = ProgressLoadingToastPosition.center;
    animationStyle = ProgressLoadingAnimationStyle.opacity;
  ...
  static ProgressLoading get instance => _instance;
  static bool get isShow => _instance.w != null;

  /// init ProgressLoading
  static TransitionBuilder init({
    TransitionBuilder? builder,
  }) {
    return (BuildContext context, Widget? child) {
      if (builder != null) {
        return builder(context, FlutterProgressLoading(child: child));
      } else {
        return FlutterProgressLoading(child: child);
      }
    };
  }
  ...
  static Future<void> show({
    ...
    final isIgnoringBackButton = !ProgressLoadingTheme.ignoring(maskType);
  ...
  static Future<void> showProgress(
    ...
    if (_instance.loadingStyle == ProgressLoadingStyle.custom) {
      assert(
    ...
    if (_instance.w == null || _instance.progressKey == null) {
      ...
      GlobalKey<ProgressLoadingProgressState> progressKey =
          GlobalKey<ProgressLoadingProgressState>();
      Widget w = ProgressLoadingProgress(
        key: progressKey,
        value: value,
      );
      _instance._show(
  ...
  static Future<void> showSuccess(
    ...
    Widget w = _instance.successWidget ??
        Icon(
          Icons.done,
          color: ProgressLoadingTheme.indicatorColor,
          size: ProgressLoadingTheme.indicatorSize,
        );
    return _instance._show(
      status: status,
      duration: duration ?? ProgressLoadingTheme.displayDuration,
      maskType: maskType,
      dismissOnTap: dismissOnTap,
      w: w,
    );
  ...
  static Future<void> showToast(
    ...
    return _instance._show(
      status: status,
      duration: duration ?? ProgressLoadingTheme.displayDuration,
      toastPosition: toastPosition ?? ProgressLoadingTheme.toastPosition,
      maskType: maskType,
      dismissOnTap: dismissOnTap,
    );
  }
  ...
  static Future<void> dismiss({
    ...
    return _instance._dismiss(animation).whenComplete(
      () {
        final isIgnoringBackButton = !ProgressLoadingTheme.ignoring(null);
  ...
  static void addStatusCallback(ProgressLoadingStatusCallback callback) {
  ...
  static void removeCallback(ProgressLoadingStatusCallback callback) {
    if (_instance._statusCallbacks.contains(callback)) {
  ...
  Future<void> _show({
    ...
    assert(
      overlayEntry != null,
      'You should call ProgressLoading.init() in your MaterialApp',
    );

    if (loadingStyle == ProgressLoadingStyle.custom) {
      assert(
    ...
    if (maskType == ProgressLoadingMaskType.custom) {
      assert(
        maskColor != null,
    ...
    if (animationStyle == ProgressLoadingAnimationStyle.custom) {
      assert(
    ...
    _key = GlobalKey<ProgressLoadingContainerState>();
    _w = ProgressLoadingContainer(
      key: _key,
      status: status,
      indicator: w,
      animation: animation,
    ...
    completer.future.whenComplete(() {
      _callback(ProgressLoadingStatus.show);
      if (duration != null) {
        _cancelTimer();
  ...
  void _reset() {
    ...
    _cancelTimer();
    _markNeedsBuild();
    _callback(ProgressLoadingStatus.dismiss);
  }

  void _callback(ProgressLoadingStatus status) {
    for (final ProgressLoadingStatusCallback callback in _statusCallbacks) {
      callback(status);
    }
  }
